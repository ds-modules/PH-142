...
---
### Importing the Data
The code below reads in the `tents.csv` comma-separated file.  The `col_types` argument is telling `R` what the data type is in each column.  See `?read_csv` if you want to know more---the `readr` package itself contains some helpful functions for parsing data.  As an aside, `read_csv` reads the data in as a `tibble`, a more modern take on the data frame.  This data structure has all the functionalities of a `data.frame`, so don't fret over it too much.  More information can be found [here](https://cran.r-project.org/web/packages/tibble/vignettes/tibble.html).

The data set itself contains information about tents collected from a particular outdoor adventure store.  The variables are as follows:

* name - name of the tent
* brand - brand of the tent
* price - price of the tent in dollars
* weight - the weight of the tent in grams
* height - the height of the tent in centimeters
* bestuse - the activity that the manufacturers recommend this tent for
* seasons - how many seasons this tent is suited for (heat/cold considerations)
* capacity - the number of people that the tent is rated for
```{r}
